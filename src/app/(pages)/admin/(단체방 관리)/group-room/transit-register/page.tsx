...
import row3Column5 from "@/data/tables/column3/row3column5";
...
const TransitRegisterPage = () => {
  ...
  const pages = Math.ceil(row3Column5.length / rowsPerPage);
  ...
  const items = React.useMemo(() => {
    ...
    setCurrentData(row3Column5.slice(start, end));
    return row3Column5.slice(start, end);
  }, [page, row3Column5, 5, rowsPerPage]);
        ...
        <article>
          ...
          >
            <TableHeader className="th-border-1">
              ...
              <TableColumn>경유지명</TableColumn>
              <TableColumn>GPS정보(N)</TableColumn>
              <TableColumn>GPS정보(E)</TableColumn>
            ...
            <TableBody>
              {items.map((row) => (
                <TableRow key={row.id} className="border-b-1">
                  ...
                  <TableCell>{row.transitPointOrder}</TableCell>
                  <TableCell className="overflow-hidden text-ellipsis whitespace-nowrap">
                    {row.gpsInfoN}
                  </TableCell>
