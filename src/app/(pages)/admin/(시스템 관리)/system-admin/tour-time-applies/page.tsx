...
import row3Column4 from "@/data/tables/column4and5/row3column4";
import React, { useState } from "react";

import MainHead from "./head";

const HolyDayRegisterInfoPage = () => {
  // Pagination Logic
  const [page, setPage] = useState(1);

  const rowsPerPage = 3;

  const pages = Math.ceil(row3Column4.length / rowsPerPage);
  ...
  const items = React.useMemo(() => {
    ...
    setCurrentData(row3Column4.slice(start, end));
    return row3Column4.slice(start, end);
  }, [page, row3Column4, 10, rowsPerPage]);
            ...
            <TableBody>
              {items.map((row) => (
                <TableRow key={row.id} className="border-b-1">
                  ...
                  <TableCell>
                    <Link
                      href={`/admin/system-admin/tour-time-applies/${row.id}`}
                      className="text-mainBlack underline-offset-2"
                    >
                      {row.tourTime}
                    </Link>
                  </TableCell>
...
export default HolyDayRegisterInfoPage;
