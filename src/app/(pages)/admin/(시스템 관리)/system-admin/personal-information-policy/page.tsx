...
import row8Column4 from "@/data/tables/column4and5/row8column4";
...
const PersonalInformationPolicy = () => {
  ...
  const pages = Math.ceil(row8Column4.length / rowsPerPage);
  ...
  const items = React.useMemo(() => {
    ...
    setCurrentData(row8Column4.slice(start, end));
    return row8Column4.slice(start, end);
  }, [page, row8Column4, 10, rowsPerPage]);
  ...
  return (
    <section className="font-noto">
      <header>
        <CommonHeader title="개인정보처리방침 등록" />
        <MainHead />
      </header>
            ...
            <TableBody>
              {items.map((row) => (
                <TableRow key={row.id} className="border-b-1">
                  ...
                  <TableCell>
                    <Link
                      href={`/admin/system-admin/personal-information-policy/${row.id}`}
