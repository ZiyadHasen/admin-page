...
import row5Column4 from "@/data/tables/column4and5/row5column4";
...
const NoticeRegisterPage = () => {
  ...
  const pages = Math.ceil(row5Column4.length / rowsPerPage);
  ...
  const items = React.useMemo(() => {
    ...
    setCurrentData(row5Column4.slice(start, end));
    return row5Column4.slice(start, end);
  }, [page, row5Column4, 10, rowsPerPage]);
        ...
        <article>
          ...
          >
            <TableHeader className="th-border-1">
              ...
              <TableColumn>공지여부</TableColumn>
              <TableColumn>등록일자</TableColumn>
              <TableColumn>공개여부</TableColumn>
              <TableColumn>공개여부</TableColumn>
            </TableHeader>
            <TableBody>
              {items.map((row) => (
                <TableRow key={row.id} className="border-b-1">
                  ...
                  <TableCell>
                    <Link
                      href={`/admin/system-admin/notice-register/${row.id}`}
                      className="text-blue underline-offset-2"
                    >
                      {row.title}
                    </Link>
                  </TableCell>
                  <TableCell className="text-red-500">{row.isNotify}</TableCell>
