...
import row9Column4 from "@/data/tables/column4and5/row9column4";
...
const RegisterMarketing = () => {
  ...
  const pages = Math.ceil(row9Column4.length / rowsPerPage);
  ...
  const items = React.useMemo(() => {
    ...
    setCurrentData(row9Column4.slice(start, end));
    return row9Column4.slice(start, end);
  }, [page, row9Column4, 10, rowsPerPage]);
            ...
            <TableBody>
              {items.map((row) => (
                <TableRow key={row.id} className="border-b-1">
                  ...
                  <TableCell>
                    <Link
                      href={`/admin/system-admin/register-marketing/${row.id}`}
