...
import row2Column4 from "@/data/tables/column4and5/row2column4";
import React, { useState } from "react";

import MainHead from "./head";

const TourTimeAppliesPage = () => {
  // Pagination Logic
  const [page, setPage] = useState(1);

  const rowsPerPage = 3;

  const pages = Math.ceil(row2Column4.length / rowsPerPage);
  ...
  const items = React.useMemo(() => {
    ...
    setCurrentData(row2Column4.slice(start, end));
    return row2Column4.slice(start, end);
  }, [page, row2Column4, 10, rowsPerPage]);
            ...
            <TableBody>
              {items.map((row) => (
                <TableRow key={row.id} className="border-b-1">
                  ...
                  <TableCell>
                    <Link
                      href={`/admin/system-admin/holiday-info-register/${row.id}`}
                      className="text-blue underline-offset-2"
                    >
                      {row.date}
                    </Link>
                  </TableCell>
                  <TableCell>{row.holydayName}</TableCell>
...
export default TourTimeAppliesPage;
